    ...
    return (function (document) {
      var doc = (function browserDoc() {
        ...
        var root = Object.assign(document.createElement('div'), {
          ...
          getStyleText() {
            var styles = [];
            for (
              var i = 0, rules = this.stylesheet.cssRules, len = rules.length;
              i < len;
              i++
            )
          ...
          toString(opt, props = selfConverter.props) {
            if (
              opt &&
              typeof opt.template != 'undefined' &&
              isFunction(opt.template)
            )
              return opt.template(this.getStyleText(), this._html(), props);
            var html = [
              '<!doctype html>\r\n<html><head><meta charset=utf-8><meta key="generator" value="docx2html"><title>' +
                (props.name || '') +
                '</title><style>',
            ];
            ...
            opt &&
              opt.extendScript &&
              html.push('<script src="' + opt.extendScript + '"></script>');
            html.push('</body><html>');
            return html.join('\r\n');
          },
          _html() {
        ...
        function indexOf(el, els) {
          for (var i = els.length - 1; i > 0; i--) if (el == els[i]) return i;
          return 0;
        }
      ...
      return (function mixin(doc) {
        ...
        return Object.assign(
          selfConverter[isNode ? 'nodefy' : 'browserify'](doc, stylesheet, opt),
          {
    ...
    return Object.assign(doc, {
      asZip(opt, props) {
        ...
        zip.file(
          'main.html',
          hasImage
            ? this.toString(opt).replace(Proto_Blob, 'images')
            : this.toString(),
        );
        return zip;
      },
      download(opt, props) {
        ...
        a.href = URL.createObjectURL(
          this.asZip(opt, props).generate({ type: 'blob' }),
        );
      ...
      save(opt, props) {
        var hasImage = false,
          ...
          me = this;
        return Promise.all(
          ((this.images && Object.keys(this.images)) || []).map(function (a) {
            hasImage = true;
          ...
          if (hasImage)
            html = html.replace(Reg_Proto_Blob, function (a, id) {
              return images[a];
            });
