...
function asStyle(x) {
  var a = [];
  for (var i in x) !isFunction(x[i]) && a.push(i.replace(AZ, r) + ':' + x[i]);
  return a.join(';');
}
...
export default class Shape extends Converter {
  ...
  convertStyle(el) {
    ...
    var style = this.wordModel.getDirectStyle(),
      propConverter = new this.constructor.Properties(
        el.style,
        this,
        pathStyle,
        bgStyle,
...
Shape.Properties = class Properties extends Style.Properties {
  ...
  gradFill(x) {
    ...
    for (var i = 0, len = x.stops.length, a; i < len; i++)
      grad.push(
        '<stop offset="' +
          (a = x.stops[i]).position +
          '%" style="stop-opacity:1;stop-color:' +
          a.color +
          '"/>',
      );
    grad.push(end);
  ...
  fillRef(x) {
    ...
    if (typeof x == 'string') this.pathStyle.fill = x;
    else if (typeof x.color != 'undefined') this.pathStyle.fill = x.color;
    else return;
    this.pathStyle.fillOpacity = 1;
  }
  fontRef(x) {
  ...
  path(x, t) {
    switch (x.shape) {
      ...
      case 'roundRect':
        this.parent.path =
          '<rect rx="' +
          (t = Math.min(this.world.width, this.world.height) / 12) +
          'pt" ry="' +
          t +
          'pt" width="' +
          this.world.width +
  ...
  vert(x) {
    ...
    this.styless(
      'transform',
      'translate(-' + delta + 'pt,' + delta + 'pt) rotate(-' + x + 'deg) ',
      this.bgStyle,
    );
